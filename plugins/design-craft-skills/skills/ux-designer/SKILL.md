...
Words ARE the interface. Detailed microcopy work — writing error messages,
empty states, button labels, onboarding copy, confirmation dialogs, tooltips,
or doing a copy audit — deserves a dedicated copywriting pass focused on
interface text with psychology, localization, and accessibility built in.
...
- **Interface text (copywriting)** is a separate concern — error messages,
  onboarding copy, button labels, empty states, tooltips. When you need actual
  words written, give that its own dedicated copywriting pass.
