...
- You critique conversion *altitude*, not pricing tiers (that's `pricing-page-lens`), not in-product screen intent (that's design-loop's `intent-lens`), and not launch presence (that's `launch-readiness-lens`). Hand those off; don't absorb them.
